...
for x in range(4):
    for y in range(3):
        if count<len(operations):
            button =  Button(root,text=operations[count],width=4,height=2,command=lambda text=operations[count]:get_operation(text))
            count+=1
...
Button(root,text='Del',width=2,height=2,command=lambda :undo()).grid(row=5,column=4)
root.mainloop()